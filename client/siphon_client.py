...
class SiphonClient:
    ...
    def request_context_call(self, context_call: ContextCall) -> str:
        """
        Send ContextCall to server for processing.
        """
        if not self.initialized:
            raise RuntimeError("SiphonClient is not initialized. Call _initialize_client first.")
    ...
    def request_synthetic_data(self, processed_content: ProcessedContent) -> SyntheticData:
        """
        ...
        if not self.initialized:
            raise RuntimeError("SiphonClient is not initialized. Call _initialize_client first.")
        response = requests.post(
            f"{self.server_url}/synthetic_data", 
            json=processed_content.model_dump_json(),
            timeout=300  # 5 min timeout for heavy processing
        )
        response.raise_for_status()
