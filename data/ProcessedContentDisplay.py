...
class ProcessedContentDisplayMixin:
    ...
    def _header_panel(self) -> Panel:
        ...
        title = self.title if self.title else f"Content from {self.uri.sourcetype.value}"
    ...
    def _uri_panel(self) -> Panel:
        ...
        table.add_row("Source Type", f"[bright_green]{self.uri.sourcetype.value}[/bright_green]")
        table.add_row("Original", f"[yellow]{self.uri.source}[/yellow]")
        table.add_row("URI", f"[blue]{self.uri.uri}[/blue]")
        
        return Panel(table, title="[bold cyan]🔗 URI[/bold cyan]", box=box.ROUNDED, style="cyan")
    ...
    def _context_panel(self) -> Panel:
        ...
        preview = context_text[:400] + "..." if len(context_text) > 400 else context_text
        
        content = Text(preview, style="white")
        content.append(f"\n\n({word_count:,} words, {char_count:,} chars)", style="dim")
        
        return Panel(content, title="[bold green]📄 Content[/bold green]", box=box.ROUNDED, style="green")
    ...
    def _metadata_panel(self) -> Panel:
        ...
        if hasattr(context, 'file_path'):
            ...
            if hasattr(context, 'content_modified_at'):
                ...
                table.add_row("Modified", f"[white]{mod_time.strftime('%Y-%m-%d')}[/white]")
        ...
        # Online metadata
            ...
            if hasattr(context, 'language'):
                table.add_row("Language", f"[bright_blue]{context.language}[/bright_blue]")
        
        return Panel(table, title="[bold bright_white]📊 Metadata[/bold bright_white]", 
                    box=box.ROUNDED, style="bright_white")
    
    def _synthetic_panel(self) -> Panel:
        """AI-generated synthetic data panel."""
        if not self.synthetic_data:
            return Panel(Text("No synthetic data", style="dim"), 
                        title="[bold magenta]🤖 AI Data[/bold magenta]", style="magenta")
        ...
        if sd.description:
            desc = sd.description[:80] + "..." if len(sd.description) > 80 else sd.description
            table.add_row("Description", f"[white]{desc}[/white]")
        
        if sd.summary:
            words = len(sd.summary.split())
            table.add_row("Summary", f"[white]{words} words[/white]")
        ...
        return Panel(table, title="[bold magenta]🤖 AI Data[/bold magenta]", 
                    box=box.ROUNDED, style="magenta")
